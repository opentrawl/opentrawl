...
- display 5 characters by default
- extend to 6 characters when 5 would collide
- display no short ref when 6 characters still collide
...
There is no authoritative mapping table; the mapping is the deterministic
function from full ref to short ref.
...
- Trawl-level mapping database: reject. It makes `open t7k3f` depend on
  ...
  stale mapping and cleanup policy.
