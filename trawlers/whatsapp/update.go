...
func (c *Crawler) Update(ctx context.Context, req *trawlkit.TrawlerCommandExecutionRequest) (*updatecontract.TrawlerArchiveUpdateReport, error) {
	...
	return &updatecontract.TrawlerArchiveUpdateReport{}, nil
}
...
func importProgress(req *trawlkit.TrawlerCommandExecutionRequest) (func(whatsappdb.ImportProgress), func()) {
	...
	if req.TrawlerCommandLog != nil {
		runProgress = req.TrawlerCommandLog.Progress(cklog.ProgressOptions{Event: "update_progress", Unit: "stage", Total: 5})
	}
	var mu sync.Mutex
	...
	report := func(event whatsappdb.ImportProgress) {
		...
		if req.ReportTrawlerCommandProgress != nil {
			req.ReportTrawlerCommandProgress(trawlkit.Progress{Phase: "update", Done: event.Done, Total: event.Total, Message: event.Message})
		}
		if runProgress != nil {
