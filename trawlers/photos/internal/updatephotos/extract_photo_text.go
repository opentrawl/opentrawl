...
func (worker *photoAssetWorker) extractPhotoText(ctx context.Context, asset archive.PhotoUpdateAsset, mediaEvidence acquiredMediaEvidence) (*cardwire.PhotoOpticalCharacterRecognition, error) {
	...
	if !found || len(response) == 0 {
		...
		if err != nil {
			if retainErr := archive.RetainPhotoModelGenerationOperationStage(ctx, runner.options.OpenedArchiveStore, asset.AssetID, inputSHA256, archive.PhotoModelGenerationPhaseTextExtraction, archive.PhotoModelGenerationStateFailed, "", "", err.Error(), time.Now()); retainErr != nil {
