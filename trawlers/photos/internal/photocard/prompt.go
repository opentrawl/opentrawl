...
- Name each spoken or written language in ordinary human words. Use an empty language list for numbers, symbols, brands or other text whose language is not visually established; never return an empty language string.
...
- The concise description identifies the main visible content in one useful sentence. Make the detailed description comprehensive: preserve every useful visible detail, composition and distinguishing image characteristic that helps a person or agent understand or retrieve the photo. Use only as many words as the image earns. Do not pad a simple image or shorten a complex one. Never exceed 500 words.
...
- POSSIBLE requires positive visible evidence for each returned place beyond generic compatibility with its category and proximity to the camera. A generic forest, street, building or landscape near a named candidate is not enough by itself. If the pixels do not distinguish any candidate or image-inferred place from other plausible surroundings, return UNKNOWN and leave the capture-location context separate.
- Select a supplied candidate using its identifier and visual evidence only. Copy supplied_candidate_identifier exactly from the checked evidence, without adding a label, prefix or other text. OpenTrawl owns its canonical human name and will insert that name mechanically. Never invent an identifier or present an image-inferred place as a supplied candidate. An image-inferred place must be genuinely absent from the supplied candidates: if its human name appears anywhere in the supplied candidates, select the supplied identifier instead and do not copy that name into the image-inferred list.
...
- Preserve every useful visible detail, composition and distinguishing image characteristic. Use only as many words as the image earns: do not pad a simple image or shorten a complex one. Never exceed 500 words.
