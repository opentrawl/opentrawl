...
## 2026-07-05 — trawlkit adoption wave (TRAWL-93), lane photoscrawl-wave
...
- **Human output rebuilt on trawlkit/render + trawlkit/usage.** search is a
...
- **Error envelope** now goes through trawlkit/output.WriteError; `--json`
...
- **place/backfill_store.go** now uses the mattn sqlite3 driver (matches
  trawlkit/store); modernc dropped from go.mod. Read-verified on the live
...
(TRAWL-82; lets trawlkit drop its *Mapped adapter). Pre-1.0, no data migration:
