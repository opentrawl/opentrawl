// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.11
// 	protoc        (unknown)
// source: trawl/app/app.proto

package app

import (
	federation "github.com/opentrawl/opentrawl/trawlkit/proto/trawl/federation"
	identity "github.com/opentrawl/opentrawl/trawlkit/proto/trawl/identity"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type ArchiveBuildPhase int32

const (
	ArchiveBuildPhase_ARCHIVE_BUILD_PHASE_UNSPECIFIED ArchiveBuildPhase = 0
	ArchiveBuildPhase_ARCHIVE_BUILD_PHASE_BUILDING    ArchiveBuildPhase = 1
	ArchiveBuildPhase_ARCHIVE_BUILD_PHASE_FINALISING  ArchiveBuildPhase = 2
)

// Enum value maps for ArchiveBuildPhase.
var (
	ArchiveBuildPhase_name = map[int32]string{
		0: "ARCHIVE_BUILD_PHASE_UNSPECIFIED",
		1: "ARCHIVE_BUILD_PHASE_BUILDING",
		2: "ARCHIVE_BUILD_PHASE_FINALISING",
	}
	ArchiveBuildPhase_value = map[string]int32{
		"ARCHIVE_BUILD_PHASE_UNSPECIFIED": 0,
		"ARCHIVE_BUILD_PHASE_BUILDING":    1,
		"ARCHIVE_BUILD_PHASE_FINALISING":  2,
	}
)

func (x ArchiveBuildPhase) Enum() *ArchiveBuildPhase {
	p := new(ArchiveBuildPhase)
	*p = x
	return p
}

func (x ArchiveBuildPhase) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (ArchiveBuildPhase) Descriptor() protoreflect.EnumDescriptor {
	return file_trawl_app_app_proto_enumTypes[0].Descriptor()
}

func (ArchiveBuildPhase) Type() protoreflect.EnumType {
	return &file_trawl_app_app_proto_enumTypes[0]
}

func (x ArchiveBuildPhase) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use ArchiveBuildPhase.Descriptor instead.
func (ArchiveBuildPhase) EnumDescriptor() ([]byte, []int) {
	return file_trawl_app_app_proto_rawDescGZIP(), []int{0}
}

type TrawlerArchiveUpdateProgress struct {
	state           protoimpl.MessageState              `protogen:"open.v1"`
	UpdatingTrawler *identity.RegisteredTrawlerIdentity `protobuf:"bytes,1,opt,name=updating_trawler,json=updatingTrawler,proto3" json:"updating_trawler,omitempty"`
	Phase           ArchiveBuildPhase                   `protobuf:"varint,2,opt,name=phase,proto3,enum=trawl.app.ArchiveBuildPhase" json:"phase,omitempty"`
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}

func (x *TrawlerArchiveUpdateProgress) Reset() {
	*x = TrawlerArchiveUpdateProgress{}
	mi := &file_trawl_app_app_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *TrawlerArchiveUpdateProgress) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TrawlerArchiveUpdateProgress) ProtoMessage() {}

func (x *TrawlerArchiveUpdateProgress) ProtoReflect() protoreflect.Message {
	mi := &file_trawl_app_app_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use TrawlerArchiveUpdateProgress.ProtoReflect.Descriptor instead.
func (*TrawlerArchiveUpdateProgress) Descriptor() ([]byte, []int) {
	return file_trawl_app_app_proto_rawDescGZIP(), []int{0}
}

func (x *TrawlerArchiveUpdateProgress) GetUpdatingTrawler() *identity.RegisteredTrawlerIdentity {
	if x != nil {
		return x.UpdatingTrawler
	}
	return nil
}

func (x *TrawlerArchiveUpdateProgress) GetPhase() ArchiveBuildPhase {
	if x != nil {
		return x.Phase
	}
	return ArchiveBuildPhase_ARCHIVE_BUILD_PHASE_UNSPECIFIED
}

type TrawlerArchiveUpdateEvent struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Types that are valid to be assigned to Kind:
	//
	//	*TrawlerArchiveUpdateEvent_Progress
	//	*TrawlerArchiveUpdateEvent_Result
	Kind          isTrawlerArchiveUpdateEvent_Kind `protobuf_oneof:"kind"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *TrawlerArchiveUpdateEvent) Reset() {
	*x = TrawlerArchiveUpdateEvent{}
	mi := &file_trawl_app_app_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *TrawlerArchiveUpdateEvent) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TrawlerArchiveUpdateEvent) ProtoMessage() {}

func (x *TrawlerArchiveUpdateEvent) ProtoReflect() protoreflect.Message {
	mi := &file_trawl_app_app_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use TrawlerArchiveUpdateEvent.ProtoReflect.Descriptor instead.
func (*TrawlerArchiveUpdateEvent) Descriptor() ([]byte, []int) {
	return file_trawl_app_app_proto_rawDescGZIP(), []int{1}
}

func (x *TrawlerArchiveUpdateEvent) GetKind() isTrawlerArchiveUpdateEvent_Kind {
	if x != nil {
		return x.Kind
	}
	return nil
}

func (x *TrawlerArchiveUpdateEvent) GetProgress() *TrawlerArchiveUpdateProgress {
	if x != nil {
		if x, ok := x.Kind.(*TrawlerArchiveUpdateEvent_Progress); ok {
			return x.Progress
		}
	}
	return nil
}

func (x *TrawlerArchiveUpdateEvent) GetResult() *federation.FederatedTrawlerArchiveUpdateOperation {
	if x != nil {
		if x, ok := x.Kind.(*TrawlerArchiveUpdateEvent_Result); ok {
			return x.Result
		}
	}
	return nil
}

type isTrawlerArchiveUpdateEvent_Kind interface {
	isTrawlerArchiveUpdateEvent_Kind()
}

type TrawlerArchiveUpdateEvent_Progress struct {
	Progress *TrawlerArchiveUpdateProgress `protobuf:"bytes,1,opt,name=progress,proto3,oneof"`
}

type TrawlerArchiveUpdateEvent_Result struct {
	Result *federation.FederatedTrawlerArchiveUpdateOperation `protobuf:"bytes,2,opt,name=result,proto3,oneof"`
}

func (*TrawlerArchiveUpdateEvent_Progress) isTrawlerArchiveUpdateEvent_Kind() {}

func (*TrawlerArchiveUpdateEvent_Result) isTrawlerArchiveUpdateEvent_Kind() {}

type SemanticSearchIndexReconcileResponse struct {
	state                        protoimpl.MessageState `protogen:"open.v1"`
	SemanticSearchIndexIsCurrent bool                   `protobuf:"varint,1,opt,name=semantic_search_index_is_current,json=semanticSearchIndexIsCurrent,proto3" json:"semantic_search_index_is_current,omitempty"`
	unknownFields                protoimpl.UnknownFields
	sizeCache                    protoimpl.SizeCache
}

func (x *SemanticSearchIndexReconcileResponse) Reset() {
	*x = SemanticSearchIndexReconcileResponse{}
	mi := &file_trawl_app_app_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SemanticSearchIndexReconcileResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SemanticSearchIndexReconcileResponse) ProtoMessage() {}

func (x *SemanticSearchIndexReconcileResponse) ProtoReflect() protoreflect.Message {
	mi := &file_trawl_app_app_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SemanticSearchIndexReconcileResponse.ProtoReflect.Descriptor instead.
func (*SemanticSearchIndexReconcileResponse) Descriptor() ([]byte, []int) {
	return file_trawl_app_app_proto_rawDescGZIP(), []int{2}
}

func (x *SemanticSearchIndexReconcileResponse) GetSemanticSearchIndexIsCurrent() bool {
	if x != nil {
		return x.SemanticSearchIndexIsCurrent
	}
	return false
}

var File_trawl_app_app_proto protoreflect.FileDescriptor

const file_trawl_app_app_proto_rawDesc = "" +
	"\n" +
	"\x13trawl/app/app.proto\x12\ttrawl.app\x1a!trawl/federation/federation.proto\x1a\x1dtrawl/identity/identity.proto\"\xa8\x01\n" +
	"\x1cTrawlerArchiveUpdateProgress\x12T\n" +
	"\x10updating_trawler\x18\x01 \x01(\v2).trawl.identity.RegisteredTrawlerIdentityR\x0fupdatingTrawler\x122\n" +
	"\x05phase\x18\x02 \x01(\x0e2\x1c.trawl.app.ArchiveBuildPhaseR\x05phase\"\xbe\x01\n" +
	"\x19TrawlerArchiveUpdateEvent\x12E\n" +
	"\bprogress\x18\x01 \x01(\v2'.trawl.app.TrawlerArchiveUpdateProgressH\x00R\bprogress\x12R\n" +
	"\x06result\x18\x02 \x01(\v28.trawl.federation.FederatedTrawlerArchiveUpdateOperationH\x00R\x06resultB\x06\n" +
	"\x04kind\"n\n" +
	"$SemanticSearchIndexReconcileResponse\x12F\n" +
	" semantic_search_index_is_current\x18\x01 \x01(\bR\x1csemanticSearchIndexIsCurrent*~\n" +
	"\x11ArchiveBuildPhase\x12#\n" +
	"\x1fARCHIVE_BUILD_PHASE_UNSPECIFIED\x10\x00\x12 \n" +
	"\x1cARCHIVE_BUILD_PHASE_BUILDING\x10\x01\x12\"\n" +
	"\x1eARCHIVE_BUILD_PHASE_FINALISING\x10\x02B=Z;github.com/opentrawl/opentrawl/trawlkit/proto/trawl/app;appb\x06proto3"

var (
	file_trawl_app_app_proto_rawDescOnce sync.Once
	file_trawl_app_app_proto_rawDescData []byte
)

func file_trawl_app_app_proto_rawDescGZIP() []byte {
	file_trawl_app_app_proto_rawDescOnce.Do(func() {
		file_trawl_app_app_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_trawl_app_app_proto_rawDesc), len(file_trawl_app_app_proto_rawDesc)))
	})
	return file_trawl_app_app_proto_rawDescData
}

var file_trawl_app_app_proto_enumTypes = make([]protoimpl.EnumInfo, 1)
var file_trawl_app_app_proto_msgTypes = make([]protoimpl.MessageInfo, 3)
var file_trawl_app_app_proto_goTypes = []any{
	(ArchiveBuildPhase)(0),                                    // 0: trawl.app.ArchiveBuildPhase
	(*TrawlerArchiveUpdateProgress)(nil),                      // 1: trawl.app.TrawlerArchiveUpdateProgress
	(*TrawlerArchiveUpdateEvent)(nil),                         // 2: trawl.app.TrawlerArchiveUpdateEvent
	(*SemanticSearchIndexReconcileResponse)(nil),              // 3: trawl.app.SemanticSearchIndexReconcileResponse
	(*identity.RegisteredTrawlerIdentity)(nil),                // 4: trawl.identity.RegisteredTrawlerIdentity
	(*federation.FederatedTrawlerArchiveUpdateOperation)(nil), // 5: trawl.federation.FederatedTrawlerArchiveUpdateOperation
}
var file_trawl_app_app_proto_depIdxs = []int32{
	4, // 0: trawl.app.TrawlerArchiveUpdateProgress.updating_trawler:type_name -> trawl.identity.RegisteredTrawlerIdentity
	0, // 1: trawl.app.TrawlerArchiveUpdateProgress.phase:type_name -> trawl.app.ArchiveBuildPhase
	1, // 2: trawl.app.TrawlerArchiveUpdateEvent.progress:type_name -> trawl.app.TrawlerArchiveUpdateProgress
	5, // 3: trawl.app.TrawlerArchiveUpdateEvent.result:type_name -> trawl.federation.FederatedTrawlerArchiveUpdateOperation
	4, // [4:4] is the sub-list for method output_type
	4, // [4:4] is the sub-list for method input_type
	4, // [4:4] is the sub-list for extension type_name
	4, // [4:4] is the sub-list for extension extendee
	0, // [0:4] is the sub-list for field type_name
}

func init() { file_trawl_app_app_proto_init() }
func file_trawl_app_app_proto_init() {
	if File_trawl_app_app_proto != nil {
		return
	}
	file_trawl_app_app_proto_msgTypes[1].OneofWrappers = []any{
		(*TrawlerArchiveUpdateEvent_Progress)(nil),
		(*TrawlerArchiveUpdateEvent_Result)(nil),
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_trawl_app_app_proto_rawDesc), len(file_trawl_app_app_proto_rawDesc)),
			NumEnums:      1,
			NumMessages:   3,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_trawl_app_app_proto_goTypes,
		DependencyIndexes: file_trawl_app_app_proto_depIdxs,
		EnumInfos:         file_trawl_app_app_proto_enumTypes,
		MessageInfos:      file_trawl_app_app_proto_msgTypes,
	}.Build()
	File_trawl_app_app_proto = out.File
	file_trawl_app_app_proto_goTypes = nil
	file_trawl_app_app_proto_depIdxs = nil
}
