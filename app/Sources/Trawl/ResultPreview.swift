...
struct ResultPreview: View {
  ...
  var body: some View {
    Group {
      ...
      case .output: if let record = response?.record { PresentationDocumentView(document: record.presentation) } else { ContentUnavailableView("Result unavailable", systemImage: "exclamationmark.circle") }
